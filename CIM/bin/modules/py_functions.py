...
class Functions:
    ...
    @_checkConnection
    def execProcedure(self, procedureName: str, values: list):
        ...
        if queryObj.exec():
            ...
            if queryObj.lastError().isValid():
                print('Error {Functions: insert}:', queryObj.lastError().text())
    ...
    def select(self, query: str) -> QSqlQuery:
        ...
        if queryObj.exec():
            ...
            if queryObj.lastError().isValid():
                print('Error {Functions: execQuery}:', queryObj.lastError().text())
                self.signals.error.emit(queryObj.lastError().text())
